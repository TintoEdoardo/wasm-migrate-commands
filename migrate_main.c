...
int main(int argc, char* argv[]) {
    ...
    printf("Acquire the semaphore...\n");
    sem_wait(&shared_obj->should_migrate);

    // Modify the should_migrate_flag.
    shared_obj->should_migrate_flag = 1;

    printf("Relinquish the semaphore...\n");
    sem_post(&shared_obj->should_migrate);
